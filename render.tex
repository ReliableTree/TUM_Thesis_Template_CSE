...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[>=stealth,font=\small]
      ...
      % Draw nodes
      \node [block] (o0) {$o_0$};
      \node [block, right of=o0, xshift=2cm] (actor) {Actor};
      \node [vector, right of=actor, xshift=2cm] (actions_vector) {$\begin{matrix}a_0\\ \vdots\\ a_T\end{matrix}$};
      
      % Draw arrows
      \draw [->, double] (o0) -- (actor);
      \draw [->, double] (actor.east) ++(0, -0.3cm) --node[midway, above] {} (actions_vector.west |- 0,-0.3cm);
      \draw [->, dashed] (o0.south) |- (actions_vector.south -| 0.3cm,0);
