...
we only optimise trajectories until critic predicts sucess probability 1
batchsize: 16
...
We use manual seed for torch and numpy. We compute multiple episodes in one run, so from seed = 0 we can get all three runs with different random behaviour. 
...
This optimisation is intractable, as we get as many constraints, as we have action state pairs in our \ac{mdp}. Moreover, in practice we only have a small sample size, which would mean setting 
