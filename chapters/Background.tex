...
\begin{equation}
    \pi^*(a \mid s) = 
    \begin{cases}
        1 & \text{if } a = \operatorname{argmax}_{a' \in \mathcal{A}} \left(\sum_{s' \in \mathcal{S}} p(s' \mid s,a) \left(r(s,a,s') + \gamma v^*(s')\right)\right) \\
        0 & \text{otherwise}
    \end{cases}
\end{equation}
...
q^*(s,a) &=   r(s,a) + \gamma \sum_{s' \in \mathcal{S}} p(s' \mid s,a) \max_{a' \in \mathcal{A}}\left[ q^*(s',a')\right].
\end{align}
\\ \\
...
\section{Actor-Critic Algorithm and Derivations}
