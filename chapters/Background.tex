...
\begin{algorithm}
    ...
    \label{algo:TQC}
    \begin{algorithmic}
        \State Initialize policy $\pi_{\varphi}$, critics $Z_{\psi_n}$, $Z_{\psi_n}$ for $n \in [1..N]$\\
        \State Set replay $D = \emptyset$, $\beta = 0.005$\\
        \For{each iteration}{
            \For{each environment step, until done}{
                \State Collect transition $(s_t, a_t, r_t, s_{t+1})$ with policy $\pi_{\varphi}$ \\
                \State $D \leftarrow D \cup {(s_t, a_t, r_t, s_{t+1})}$\\
            \EndFor}
            \For{each gradient step}{
                \State Sample a batch from the replay $D$\\
                \State $\varphi \leftarrow \varphi - \lambda{\pi} \hat{\nabla}{\varphi}J{\pi}(\varphi)$\\
            ...
            \EndFor\\}
        \EndFor\\}
        \State \textbf{return} policy $\pi{\varphi}$, critics $Z_{\psi_n}$
    \end{algorithmic}
\end{algorithm}
