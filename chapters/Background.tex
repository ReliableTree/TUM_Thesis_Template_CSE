...
To update the policy, the SAC algorithm uses a state-action value estimate $Q$ which is learned using a state value estimate $V$ like in \ref{q_from_v}. 
$V$ is learned with the minimum of two separate critics $Q_1^\theta(s,a)$ and $Q_2^\phi(s,a)$ 
...
In a similar way, we approximate $\mathbb{H}(\pi) + \mathbb{E}_{\pi}[r(s, a)]$.\\
...
\end{equation}
, where $D$ is the discriminator used is $\psi^*$.
...
The algorithm is summarized in \ref{GAIL_Algo}. Let $\pi_E = \pi_{r'}^*$ be the expert policy, which reward function $r'$ we want to imitate. 
\begin{algorithm}
...
\end{figure}
Dies ist ein Test, wie das jetzt aussihet
\subsection{Symmetry}