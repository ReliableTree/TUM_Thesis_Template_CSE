...
achieve this is by minimizing the KL-Divergence between the normalized $Q$-Value and the policy $\pi$. 
\begin{equation}
    \label{kl-obj}
    \begin{aligned}
...
\subsection{TQC}
To derive TQC, we start with the distributional reinforcement learning algorithm (DRL) and build upon it to mitigate the overestimation bias. In DRL, the Q-value function is represented by a distribution $Z(s, a, z)$, where $s$ and $a$ denote the state and action, and $z$ denotes a sample from the distribution. The distribution is learned using a critic network that minimizes the Kullback-Leibler (KL) divergence between the true value distribution $Z^{\pi}(s,a)$ and the estimated distribution $\hat{Z}(s,a,z)$:

$$\mathcal{L}(\theta) = \text{KL}(Z^{\pi}(s,a) || \hat{Z}(s,a,z; \theta)),$$

where $\theta$ denotes the parameters of the critic network.

To address the issue of overestimation bias, TQC introduces a truncated mixture of continuous distributions to estimate the upper bound of the Q-value distribution. The truncated mixture is defined as follows:

$$Z_T(s, a, z) = \sum_{i=1}^{n}\alpha_i(s, a)Z_i(s, a, z),$$

where $n$ denotes the number of mixture components, $\alpha_i(s, a)$ denotes the mixing coefficient of the $i$-th component, and $Z_i(s, a, z)$ denotes the distribution of the Q-value function for the $i$-th component. The mixing coefficients and the distributions are learned using the critic network.

To truncate the upper bound of the distribution, TQC introduces a truncation function $\tau(s,a)$, which maps the upper bound of the distribution to a fixed value $T$. The truncated distribution is defined as follows:

$$Z_T(s, a, z) = \min(Z(s, a, z), \tau(s,a)),$$

where $\tau(s,a)$ is a learned function of the state and action.

The loss function for TQC is similar to DRL, but it also includes a penalty term to encourage the critic network to learn a lower bound for the value distribution:

$$\mathcal{L}(\theta) = \text{KL}(Z^{\pi}(s,a) || \hat{Z}T(s,a,z; \theta)) + \lambda \mathbb{E}{z\sim \hat{Z}_T(s,a,z; \theta)}[\max(0, V - z)],$$

where $V$ denotes a fixed upper bound for the Q-value function, and $\lambda$ is a hyperparameter that controls the strength of the penalty term. The penalty term encourages the critic network to learn a lower bound for the Q-value function, which helps to further reduce the overestimation bias.

In summary, TQC addresses the issue of overestimation bias in value-based reinforcement learning algorithms by introducing a truncated mixture of continuous distributions to estimate the upper bound of the Q-value distribution. The truncation function maps the upper bound to a fixed value, and a penalty term encourages the critic network to learn a lower bound for the Q-value distribution. TQC has shown promising results in various tasks and has the potential to improve the performance of value-based reinforcement learning algorithms.
