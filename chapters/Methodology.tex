...
\end{equation}
. This is an unbiased estimate, as it is simply the empiciral distribution over the rewards given an observation o and an action sequence $a_{1,...,T}$. As the 
first observation is all the information we have during inference time, we find this aproach to be well suited for our setup. As the critic predicts the reward 
for the whole sequence of steps into the future, it is an estimate of a proper value equvialent model of the POMDP, as defined by Christopher Grimm et. al (2021) \ref{grimm2021proper}. 
[[[make this part of backgournd, then say what happens here]]]. Following this insight, we know we can learn an optimal policy using this critic.
