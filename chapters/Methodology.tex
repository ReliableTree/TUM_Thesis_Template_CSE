...
Thus the dimension of the preimage of the actor is $n \times m$ with $\pi:\mathcal{R}^m \times \mathcal{R}^n \rightarrow \mathcal{R}$. Similarly, 
the critic $Q$ usually defines a function on $\mathcal{A}$ and $\mathcal{O}$ to the cumulative discounted expected value 
...
\end{equation}
\begin{equation*}
    = \left(\nabla_{\phi} J(\phi, \theta(\phi) + \epsilon) - \widetilde{\nabla}_\phi J(\phi, \theta + \epsilon)\right) - \epsilon_{\widetilde{\nabla}_{\theta}}.
\end{equation*}
We will call 
\begin{equation}
...
where we use parameters $\theta$ for the critic. It can be seen, that $C_{\theta, s_1, a_{1:T}}$ has no dependency of $\pi$, as 
the action sequence is completely part of the argument. From this simple fact, we 
...
Recall, the function approximator error $\epsilon_{\widetilde{\nabla}_{\theta}}$ defined in Equation \ref{eq:def_func_app_err} was 
...
To do this, we propose to use a planning and search based method using the critic in inference time.

Let $a = \pi(s)$ be the planned action sequence proposed by the actor and $C_{\theta, s, a_{1:T}}(t)$ be the predicted rewards by the critic. 
...
through C and thus not a successful trajectory. To prevent this problem, we will disambiguate the actions $a_1$ and $a_2$. We propose an additional network we call the planner $P$. $P$ takes 
