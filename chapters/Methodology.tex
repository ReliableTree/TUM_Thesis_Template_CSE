...
In the following chapter, we will first develop the active critic (AC) algorithm defined on single observation POMDPs and then relax 
the constraint to the MDP setting. To indicate an observation after each step, we will refer to the MDP setting as "continuous observation" 
and call the AVC algorithm for MDPs "Continuous Observation Active Critic" (COAC).\\
...
Formally, let $o_{1:t}$ be the observations until timestep $t$ and let $a:{1:T}$ be an action sequence, where $a:{1:t}$ are the actions already taken until timestep $t$. 
We define a whole sequence actor as a model, that takes $o_{1:t}$ as input and returns actions $a:{1:T}: \pi(o_{1:t}) = a_{1:T}$. Similar, we define a whole sequence critic 
as $C(o_{1:t}, a_{1:T}) = c_{1:T}$, where $c_t$ is the expected immediate reward at timestep $t$. 
...
We call this a strong inductive belief state bias for the SOPOMDP, as it forces the assumption of a deterministic and constant policy $\pi$. Here we see, 
