...
\section{Reinforcement Learning from Sparse Rewads in Single Observation Environments}
In this section we will explore the perforance of the AC algorithm given sparse rewards from trajectories together with expert demonstrations. We find this to be 
a plausible setting, as sparse rewards are typically easy to define even in real world applications or sparse reward from human feedback could be used. It is well 
discussed that reward shaping is tedious and can lead to sub optimal perforance. As an example, imagine an automated production line, where a robot conducts a 
...
not trivial to do so in a way that is helpful for the learning algorithm. Another application are natural language models. Resent efforts have been made to tune 
models according to humane preferences from a pretrained initialisation. Language models don't get any observations from the environment, as the initial 
...
In this sense, it is comparable to our setup. We will focus on dense action and observation spaces however and leave further investigation of discrete spaces to future work. 
In addition to the practical relevance of the setting, an advantage is that the planning part of our algorithm is most efficient in single observation settings. 
...
of tasks and differently levels suited to test different aspects of our algorithm. We defined a constant sequence length of 100 steps per trajectory and provided 
...
For the base implementations we use "Stable Baselines3" \ref{stable-baselines3}, which is a common choice, as it provides well implemted algorithms with tested 
...
We expected RPPO to be the least performant from our discussion of the curse of dimensionality in section \ref{COD_AC}. As no current input is provided from the enviroment, RPPO has to keep track of 
