...
First we will compare pure imitation learning of our algorithm in the deterministic single observation POMDP (SOPOMDP) of the 
...
In summary, we will first investigate AVC on pure imitation learning, then we will test AVC in the deterministic SOPOMDP setting and finally we analyze DOAVC in the MDP setting.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{
  All learners were pretrained using behavioural cloning with the given number of expert demonstrations. 
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{
    All learners were pretrained using behavioural cloning with the given number of expert demonstrations, where demonstrations were provided. 
...
Next, we wanted to rule out that the observed performance gain of AVC over the baselines comes mostly from increased perforamce in imitation learning 
...
figure \ref{fig:guided_ref}. We find AVC significantly benefits from the reinforcement phase over pure imitation learning. \\
Overall, we take these finding as evidence for the effectiveness of the reinforcement aspect of the AVC algorithm.
