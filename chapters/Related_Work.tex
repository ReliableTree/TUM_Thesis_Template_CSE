...
\section{Test}
Let $V$ be the value function estimate and $V^{\pi}$ be the true value function for policy $\pi$. We want to show that $V$ converges to $V^{\pi}$ under the action of the update operator $T'$.
...
\begin{aligned}
|(T'V)(s) - (T'W)(s)|\infty \\
&= \max{s \in S}\left|E_{a \in A \propto \pi}\left(r(s,a) + \gamma \sum_{s' \in S} p(s' \mid s,a) V(s')\right) - \\
E_{a \in A \propto \pi}\left(r(s,a) + \gamma \sum_{s' \in S} p(s' \mid s,a) W(s')\right)\right| \\
&\leq \max_{s \in S}\gamma \sum_{s' \in S} p(s' \mid s,a) |V(s') - W(s')| \\
&= \gamma |V - W|_\infty
\end{aligned}

where the inequality follows from the triangle inequality and the fact that $|V(s') - W(s')| \geq 0$.

Therefore, $T'$ is a contraction mapping with respect to the infinity norm, and by the Banach fixed point theorem, it has a unique fixed point $V^{\pi}$.

Step 2: Limit

Let $V_k$ be the value function estimate at iteration $k$, and let $V^{\pi}$ be the unique fixed point of $T'$. Then we have:

\begin{align}
|V_{k+1} - V^{\pi}|\infty &= |(T'V_k) - T'V^{\pi}|\infty \
&\leq \gamma |V_k - V^{\pi}|_\infty
\end{align}

where the inequality follows from the contraction property of $T'$.

Iterating this inequality $k$ times, we get:

\begin{align}
|V_{k+1} - V^{\pi}|\infty &\leq \gamma^k |V_0 - V^{\pi}|\infty \
&\leq \gamma^k \max_{s \in S}|V_0(s) - V^{\pi}(s)|
\end{align}

Since $\gamma < 1$, we have $\gamma^k \rightarrow 0$ as $k \rightarrow \infty$, which implies that $V_k \rightarrow V^{\pi}$ as $k \rightarrow \infty$. Moreover, the convergence is uniform, i.e., $|V_k - V^{\pi}|_\infty \rightarrow 0$ as $k \rightarrow \infty$.

Therefore, the value estimate converges to the true value function $V^{\pi}$, which is the optimal value function for policy $\pi$.